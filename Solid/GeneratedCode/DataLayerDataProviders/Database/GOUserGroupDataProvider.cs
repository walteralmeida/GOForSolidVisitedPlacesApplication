...
{
    public class GOUserGroupDataProvider : DatabaseDataProvider<GOUserGroupDataObject>
    {
        ...
        protected override GOUserGroupDataObject DoGetFromDatabase(
			GOUserGroupDataObject entity, 
			...
			Parameters parameters)
        {
			var securityFilter = securityFilterExpression as Expression<Func<ORMGOUserGroup, bool>>;

			var query = Query(
				e => e.GOGroupName == entity.GOGroupName && e.GOUserId == entity.GOUserId, 
				null,
				null,
				securityFilter, 
			...
			ApplicationSettings.TryResolve<IPrefetch<ORMGOUserGroup>>()?.Fetch(query, includes, parameters);

			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var result = query.FirstOrDefault()?.ToDataObject(dataset) as GOUserGroupDataObject;

			return result;
        }

        // filterExpression is used to filter data, when filter is statically known. dynamicFilterExpression is used for dynamic filtering, when filter is not known at compile time. Both can be used at the same time
        protected override DataObjectCollection<GOUserGroupDataObject> DoGetCollectionFromDatabase(
			LambdaExpression filterExpression, 
			string dynamicFilterExpression, 
			object[] dynamicFilterArguments, 
			...
			Parameters parameters)
        {
			...
			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var collection = query.Select(x => x.ToDataObject(dataset)).Cast<GOUserGroupDataObject>().ToList();
			return new DataObjectCollection<GOUserGroupDataObject>(collection);
        }

        protected override GOUserGroupDataObject DoSaveToDatabase(
			GOUserGroupDataObject entity, 
			List<string> includes, 
			IObjectsDataSet context,
			Parameters parameters)
		...
		private IQueryable<ORMGOUserGroup> Query(
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
		...
		private IQueryable<ORMGOUserGroup> Query(
			Expression<Func<ORMGOUserGroup, bool>> selectPredicate,
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
			LambdaExpression compiledFilter,
			bool throwIfAccessDenied = false)
		{
			...
			IQueryable<ORMGOUserGroup> query = null;

			if (!String.IsNullOrWhiteSpace(dynamicFilterExpression))
			{
				Expression<Func<ORMGOUserGroup, bool>> dynamicPredicate = null;
 
				query = session.Query<ORMGOUserGroup>();

				if (selectPredicate != null)
					query = query.Where(selectPredicate);

				dynamicPredicate = System.Linq.Dynamic.DynamicExpression.ParseLambda<ORMGOUserGroup, bool>(dynamicFilterExpression, dynamicFilterArguments);
		...
		private IQueryable<ORMGOUserGroup> ApplyCompileTimeFilterExpression(
			IQueryable<ORMGOUserGroup> query, 
			LambdaExpression filterExpression, 
			bool throwIfAccessDenied)
		{
			// Apply filter, and if it's a security filter throw exception if no results after applying it
			var filter = filterExpression as Expression<Func<ORMGOUserGroup, bool>>;
				...
				if (query.Any())
				{
					...
					if (!filtered.Any())
					{
						if (throwIfAccessDenied)
						{
							...
							auth.ThrowAccessDenied(new GOServerException("accessDenied", auth.ExplainAccessDenied(EntityAccessEnum.READ, "GOUserGroup")));
						}
					}
