...
{
    public class GOGroupRoleDataProvider : DatabaseDataProvider<GOGroupRoleDataObject>
    {
        ...
        protected override GOGroupRoleDataObject DoGetFromDatabase(
			GOGroupRoleDataObject entity, 
			...
			Parameters parameters)
        {
			var securityFilter = securityFilterExpression as Expression<Func<ORMGOGroupRole, bool>>;

			var query = Query(
				e => e.GOGroupName == entity.GOGroupName && e.GORoleName == entity.GORoleName, 
				null,
				null,
				securityFilter, 
			...
			ApplicationSettings.TryResolve<IPrefetch<ORMGOGroupRole>>()?.Fetch(query, includes, parameters);

			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var result = query.FirstOrDefault()?.ToDataObject(dataset) as GOGroupRoleDataObject;

			return result;
        }

        // filterExpression is used to filter data, when filter is statically known. dynamicFilterExpression is used for dynamic filtering, when filter is not known at compile time. Both can be used at the same time
        protected override DataObjectCollection<GOGroupRoleDataObject> DoGetCollectionFromDatabase(
			LambdaExpression filterExpression, 
			string dynamicFilterExpression, 
			object[] dynamicFilterArguments, 
			...
			Parameters parameters)
        {
			...
			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var collection = query.Select(x => x.ToDataObject(dataset)).Cast<GOGroupRoleDataObject>().ToList();
			return new DataObjectCollection<GOGroupRoleDataObject>(collection);
        }

        protected override GOGroupRoleDataObject DoSaveToDatabase(
			GOGroupRoleDataObject entity, 
			List<string> includes, 
			IObjectsDataSet context,
			Parameters parameters)
		...
		private IQueryable<ORMGOGroupRole> Query(
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
		...
		private IQueryable<ORMGOGroupRole> Query(
			Expression<Func<ORMGOGroupRole, bool>> selectPredicate,
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
			LambdaExpression compiledFilter,
			bool throwIfAccessDenied = false)
		{
			...
			IQueryable<ORMGOGroupRole> query = null;

			if (!String.IsNullOrWhiteSpace(dynamicFilterExpression))
			{
				Expression<Func<ORMGOGroupRole, bool>> dynamicPredicate = null;
 
				query = session.Query<ORMGOGroupRole>();

				if (selectPredicate != null)
					query = query.Where(selectPredicate);

				dynamicPredicate = System.Linq.Dynamic.DynamicExpression.ParseLambda<ORMGOGroupRole, bool>(dynamicFilterExpression, dynamicFilterArguments);
		...
		private IQueryable<ORMGOGroupRole> ApplyCompileTimeFilterExpression(
			IQueryable<ORMGOGroupRole> query, 
			LambdaExpression filterExpression, 
			bool throwIfAccessDenied)
		{
			// Apply filter, and if it's a security filter throw exception if no results after applying it
			var filter = filterExpression as Expression<Func<ORMGOGroupRole, bool>>;
				...
				if (query.Any())
				{
					...
					if (!filtered.Any())
					{
						if (throwIfAccessDenied)
						{
							...
							auth.ThrowAccessDenied(new GOServerException("accessDenied", auth.ExplainAccessDenied(EntityAccessEnum.READ, "GOGroupRole")));
						}
					}
