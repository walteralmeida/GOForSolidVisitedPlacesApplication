...
{
    public class UserProfileDataProvider : DatabaseDataProvider<UserProfileDataObject>
    {
		...
		private IQueryable<ORMUserProfile> ApplyCompileTimeFilterExpression(
			...
			bool throwIfAccessDenied)
		{
			...
			var filter = filterExpression as Expression<Func<ORMUserProfile, bool>>;
				...
				if (query.Any())
				{
					...
					if (!filtered.Any())
					{
						if (throwIfAccessDenied)
						{
							...
							auth.ThrowAccessDenied(new GOServerException("accessDenied", auth.ExplainAccessDenied(EntityAccessEnum.READ, "UserProfile")));
						}
					}
