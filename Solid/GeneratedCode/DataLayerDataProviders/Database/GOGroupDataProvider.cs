...
{
    public class GOGroupDataProvider : DatabaseDataProvider<GOGroupDataObject>
    {
        ...
        protected override GOGroupDataObject DoGetFromDatabase(
			GOGroupDataObject entity, 
			...
			Parameters parameters)
        {
			var securityFilter = securityFilterExpression as Expression<Func<ORMGOGroup, bool>>;
			...
			ApplicationSettings.TryResolve<IPrefetch<ORMGOGroup>>()?.Fetch(query, includes, parameters);

			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var result = query.FirstOrDefault()?.ToDataObject(dataset) as GOGroupDataObject;

			return result;
        }

        // filterExpression is used to filter data, when filter is statically known. dynamicFilterExpression is used for dynamic filtering, when filter is not known at compile time. Both can be used at the same time
        protected override DataObjectCollection<GOGroupDataObject> DoGetCollectionFromDatabase(
			LambdaExpression filterExpression, 
			string dynamicFilterExpression, 
			object[] dynamicFilterArguments, 
			...
			Parameters parameters)
        {
			...
			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var collection = query.Select(x => x.ToDataObject(dataset)).Cast<GOGroupDataObject>().ToList();
			return new DataObjectCollection<GOGroupDataObject>(collection);
        }

        protected override GOGroupDataObject DoSaveToDatabase(
			GOGroupDataObject entity, 
			List<string> includes, 
			IObjectsDataSet context,
			Parameters parameters)
		...
		private IQueryable<ORMGOGroup> Query(
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
		...
		private IQueryable<ORMGOGroup> Query(
			Expression<Func<ORMGOGroup, bool>> selectPredicate,
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
			LambdaExpression compiledFilter,
			bool throwIfAccessDenied = false)
		{
			...
			IQueryable<ORMGOGroup> query = null;

			if (!String.IsNullOrWhiteSpace(dynamicFilterExpression))
			{
				Expression<Func<ORMGOGroup, bool>> dynamicPredicate = null;
 
				query = session.Query<ORMGOGroup>();

				if (selectPredicate != null)
					query = query.Where(selectPredicate);

				dynamicPredicate = System.Linq.Dynamic.DynamicExpression.ParseLambda<ORMGOGroup, bool>(dynamicFilterExpression, dynamicFilterArguments);
		...
		private IQueryable<ORMGOGroup> ApplyCompileTimeFilterExpression(
			IQueryable<ORMGOGroup> query, 
			LambdaExpression filterExpression, 
			bool throwIfAccessDenied)
		{
			// Apply filter, and if it's a security filter throw exception if no results after applying it
			var filter = filterExpression as Expression<Func<ORMGOGroup, bool>>;
				...
				if (query.Any())
				{
					...
					if (!filtered.Any())
					{
						if (throwIfAccessDenied)
						{
							...
							auth.ThrowAccessDenied(new GOServerException("accessDenied", auth.ExplainAccessDenied(EntityAccessEnum.READ, "GOGroup")));
						}
					}
