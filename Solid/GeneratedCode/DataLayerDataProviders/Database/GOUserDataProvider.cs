...
{
    public class GOUserDataProvider : DatabaseDataProvider<GOUserDataObject>
    {
        ...
        protected override GOUserDataObject DoGetFromDatabase(
			GOUserDataObject entity, 
			...
			Parameters parameters)
        {
			var securityFilter = securityFilterExpression as Expression<Func<ORMGOUser, bool>>;
			...
			ApplicationSettings.TryResolve<IPrefetch<ORMGOUser>>()?.Fetch(query, includes, parameters);

			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var result = query.FirstOrDefault()?.ToDataObject(dataset) as GOUserDataObject;

			return result;
        }

        // filterExpression is used to filter data, when filter is statically known. dynamicFilterExpression is used for dynamic filtering, when filter is not known at compile time. Both can be used at the same time
        protected override DataObjectCollection<GOUserDataObject> DoGetCollectionFromDatabase(
			LambdaExpression filterExpression, 
			string dynamicFilterExpression, 
			object[] dynamicFilterArguments, 
			...
			Parameters parameters)
        {
			...
			var dataset = ApplicationSettings.Resolve<IObjectsDataSet>();
			var collection = query.Select(x => x.ToDataObject(dataset)).Cast<GOUserDataObject>().ToList();
			return new DataObjectCollection<GOUserDataObject>(collection);
        }

        protected override GOUserDataObject DoSaveToDatabase(
			GOUserDataObject entity, 
			List<string> includes, 
			IObjectsDataSet context,
			Parameters parameters)
		...
		private IQueryable<ORMGOUser> Query(
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
		...
		private IQueryable<ORMGOUser> Query(
			Expression<Func<ORMGOUser, bool>> selectPredicate,
			string dynamicFilterExpression,
			object[] dynamicFilterArguments,
			LambdaExpression compiledFilter,
			bool throwIfAccessDenied = false)
		{
			...
			IQueryable<ORMGOUser> query = null;

			if (!String.IsNullOrWhiteSpace(dynamicFilterExpression))
			{
				Expression<Func<ORMGOUser, bool>> dynamicPredicate = null;
 
				query = session.Query<ORMGOUser>();

				if (selectPredicate != null)
					query = query.Where(selectPredicate);

				dynamicPredicate = System.Linq.Dynamic.DynamicExpression.ParseLambda<ORMGOUser, bool>(dynamicFilterExpression, dynamicFilterArguments);
		...
		private IQueryable<ORMGOUser> ApplyCompileTimeFilterExpression(
			IQueryable<ORMGOUser> query, 
			LambdaExpression filterExpression, 
			bool throwIfAccessDenied)
		{
			// Apply filter, and if it's a security filter throw exception if no results after applying it
			var filter = filterExpression as Expression<Func<ORMGOUser, bool>>;
				...
				if (query.Any())
				{
					...
					if (!filtered.Any())
					{
						if (throwIfAccessDenied)
						{
							...
							auth.ThrowAccessDenied(new GOServerException("accessDenied", auth.ExplainAccessDenied(EntityAccessEnum.READ, "GOUser")));
						}
					}
