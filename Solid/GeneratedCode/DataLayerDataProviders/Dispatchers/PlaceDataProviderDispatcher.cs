...
namespace Solid.Data.DataProviders.Dispatchers
{
    public class PlaceDataProviderDispatcher : IDataProviderDispatcher<PlaceDataObject>
    {
        ...
        public void DispatchForEntity(PlaceDataObject entity, List<string> includes, IObjectsDataSet context, Dictionary<string, object> parameters, bool skipSecurity = false)
        {
			...
			var prefetches = PrefetchAssociations.Get("Place", parameters);
                  ...
                  case "placetolocationitems":
							{
								...
								try
								{
									var objectToFetch = PlaceToLocationDataProvider.GetCollection(null, String.Format("PlaceURI == \"{0}\"", entity.URI), null, null, 0, 0, subincludes, context, parameters, skipSecurity);
									if (objectToFetch != null) 
									{
										entity.ObjectsDataSet.Merge(objectToFetch.ObjectsDataSet);
									}
						...
						default:
							throw new ApplicationException("Place Entity has no relation named " + relation);
					}
          }        
		}

        public void DispatchForEntityCollection(IEnumerable<PlaceDataObject> entities, List<string> includes, IObjectsDataSet context, Dictionary<string, object> parameters, bool skipSecurity = false)
        {
			// Remember includes we've already dispatched so as to avoid multiple data fetches
			var dispatched = new HashSet<string>();
            ...
            foreach (var include in includes)
            {
					...
					switch (relation)
					{
						...
						case "placetolocationitems":
                        {
							...
							try
							{
								entities.First().ObjectsDataSet.Merge(PlaceToLocationDataProvider.GetCollection(null, "(@0.Contains(outerIt.PlaceURI))", filterparameters, null, 0, 0, subincludes, context, parameters, skipSecurity).ObjectsDataSet);
							}
							catch (GOServerException e)
							{
								if (e.Reason != "accessDenied")
