...
(function () {
	// 
	Solid.Web.Controllers.PlacePageController = function(applicationController) {
		var self = this;
		this.subscriptions = [];
		// store all subscriptions in this array, to unsubscribe on release.
		
		this._objectType = "PlacePage";
		...
		if (Solid.Web.Controllers.PlacePageControllerCustom !== undefined) {
		    this.customController = new Solid.Web.Controllers.PlacePageControllerCustom(self);
		};

		this.pageTitle = ko.observable("Place Details Page");

		// Initialize View Models and Data Stores
		
			this.PlaceViewModel = new Solid.Web.ViewModels.PlaceFormViewModel(this, $("#PlaceFormControl"), null, null, this.contextId);		
	
		// Attach to view models events
		this.subscriptions.push(this.PlaceViewModel.StatusData.IsBusy.subscribe( function (newValue) { self.OnPlaceViewModelIsBusyChanged(newValue); }));
		
		this.IsInEditMode = function() {

			return  (self.PlaceViewModel.StatusData.DisplayMode && self.PlaceViewModel.StatusData.DisplayMode() == 'edit');
		};


		// Events Handlers
		this.OnPlaceViewModelIsBusyChanged = function (newValue) {
		};
		...
		this.initialize = function() {
			...
			if (partsCount == 4) {
				...
				if(allPksValid) {
					var objectToLoad = new Solid.Web.Model.DataObjects.PlaceObject();
		...
		this.release = function() {
			...
			self.PlaceViewModel.release();
			self.PlaceViewModel = null;
		};
	...
	if (window.ApplicationSourceHandler)
		window.ApplicationSourceHandler.onSourceLoaded("/Controllers/PlacePageController.js");
} ());
