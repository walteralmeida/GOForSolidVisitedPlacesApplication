...
(function () {
	//
	Solid.Web.ViewModels.LocationGridViewModel = function(controller, $gridContainer, sDataBindRoot, $popupContainer, parentContextId, options) {
		var self = this;
		...
		// Object Data
		this.LocationObjectCollection = ko.observableArray();

		self.subscriptions.push(self.LocationObjectCollection.subscribe( function (newValue) {
		...
		this.LocationFilterViewModel = new Solid.Web.ViewModels.Filters.LocationFilterViewModel(self.controller, null, null, self.contextId);

		
        this.FILTER_NAME = "LocationFilter";  //Persitent filter
		...
		this.onLocationFilterViewModelSearch = function () {
		    var predicate = self.LocationFilterViewModel.getFilterPredicate();
		...
		this.onLocationFilterViewModelClear = function () {
		    self.clearFilterPredicate();
		};

		// Subscriptions to filter changes
		self.subscriptions.push(this.LocationFilterViewModel.events.onSearch.subscribe(function (newValue) { self.onLocationFilterViewModelSearch(); }));
		self.subscriptions.push(this.LocationFilterViewModel.events.onClear.subscribe(function (newValue) { self.onLocationFilterViewModelClear(); }));
		...
		this.selectedObject = ko.pureComputed(function () {
            if (this.selectedObjectId() == -1)
                return null;
            return this.controller.ObjectsDataSet.GetObjectByInternalId("Location", this.selectedObjectId());
        }, this);
        ...
        this.StatusData = {
            ...
            IsEmpty: ko.pureComputed( function () {
                return !self.LocationObjectCollection() || self.LocationObjectCollection().length === 0;
            }),
		...
		if (Solid.Web.ViewModels.LocationGridViewModelCustom !== undefined) {
		    this.customViewModel = new Solid.Web.ViewModels.LocationGridViewModelCustom(self);
		};
		...
		// Events
        this.Events = {
			LocationCollectionLoaded: ko.observable(false),
		...
		this.showCreateNewPopup = function () {
            ...
            var newObject = Solid.Web.Model.DataObjects.LocationObjectFactory.createNew(self.controller.ObjectsDataSet, null);
		    ...
		    GO.log("LocationGrid", "Creating entity opening AccountForm");

		    var memoryOnly = self.isMemoryOnlyCollection || self.StatusData.DisplayMode() === 'edit';
            ApplicationController.showCreateNewPopup("LocationForm", self, newObject, memoryOnly, self.contextId, '70%');
		};
		this.onCurrentDeleted = function () {
		    GO.log("LocationGrid", "Location deleted");
			self.Rebind(true);
            self.StatusData.IsBusy(false);
			ko.postbox.publish('Location.ChangedOnGrid', { contextId: self.contextId, action: 'delete', gridName: 'LocationGrid'  });
		};
		...
		this.onConfirmDeleteCurrent = function (confirm) {
            if (confirm) {
                if (!self.isMemoryOnlyCollection) {
					...
					GO.log("LocationGrid", "Deletion confirmed, sending query to server", { URI: objectToDelete.Data.URI() });
					...
					var index = self.LocationObjectCollection.indexOf(self.selectedObject());
					if (index > -1) {
						self.LocationObjectCollection.splice(index, 1);
					}
					// else try to match based on PKs
					else {
						for (var i = 0; i < self.LocationObjectCollection().length; ++i) {
							if (
								self.LocationObjectCollection()[i].Data.URI() == self.selectedObject().Data.URI()  
							) {
								self.LocationObjectCollection.splice(i, 1);
								break;
							}
						}
					}
		...
		this.deleteCurrent = function() {
		    GO.log("LocationGrid", "Asking to delete Location");
			if (self.commands.isDeleteCommandEnabled() === false)
				return;

            self.StatusData.IsBusy(true);

			ApplicationController.showConfirmPopup(self, Solid.Web.Messages.confirmDeleteMessage.replace(/%ENTITY%/g, "Location"), Solid.Web.Messages.confirmDeletePopupTitle, self.onConfirmDeleteCurrent, self.contextId);
		};
		this.CallAfterSaveRelatedEntity = function () {
			ko.postbox.publish('Location.ChangedOnGrid', { contextId: self.contextId, action: 'add', gridName: 'LocationGrid' });
		}
        ...
        this.SetLocationObjectCollection = function(dataObjectCollection) {
		    GO.log("LocationGrid", "Setting new collection");

            self.selectedObjectId(null);

			var currentCollection = self.LocationObjectCollection();
            ...
            if(!GenerativeObjects.Web.GetEnvironment().isMobile) {// mobile infinite lists need to keep the full list of objects
				...
				self.LocationObjectCollection.removeAll();
			}
		...
		this.Rebind = function(forceExternal) {
			...
			// If we're only working in memory, we simulate a Rebind by reloading observables
				...
				if (self.getSourceCollection) {
					...
					self.SetLocationObjectCollection(allObjects);
				}
            }
		...
		this.LoadLocationObjectCollection = function (configuration) {
			if (!self.DataStore)		
				return;
			...
			var configuration = $.merge({
				...
				successHandler:self.OnLocationObjectCollectionCounted,
		    ...
		    GO.log("LocationGrid", "Calling count");
			self.DataStore.CountObjects(configuration);
        };

		this.OnLocationObjectCollectionCounted = function (count) {
		    GO.log("LocationGrid", count + " elements counted");
			...
			self.LoadPagedLocationObjectCollection();
		};
		this.LoadPagedLocationObjectCollection = function (configuration) {
			if (!self.DataStore)		
				return;
			...
			var configuration = $.merge({
				...
				successHandler:self.OnLocationObjectCollectionLoaded,
				errorHandler: self.ShowError
	        }, configuration || {} );
		    ...
		    GO.log("LocationGrid", "Calling load");
        ...
        this.OnLocationObjectCollectionLoaded = function (objectsLoaded) {
		    GO.log("LocationGrid", "LocationCollection loaded");
            self.SetLocationObjectCollection(objectsLoaded);
			self.StatusData.IsBusy(false);
            // the next line is to force notification of change: this way we emulate event handling
            self.Events.LocationCollectionLoaded(!self.Events.LocationCollectionLoaded());
		...
		this.gridSettings = {
			...
			dataCollectionName: 'LocationObjectCollection',
			dataCollectionObservable: self.LocationObjectCollection,
        	...
        	self.subscriptions.push(self.gridSettings.sortingOptions.subscribe(
        	    function( oSort ) {
                  ...
                  if (self.totalPageNumber() < 2 && !self.multiSortOrderBy) {
						self.LocationObjectCollection.sort(GO.getSortFunction(oSort.columnName, oSort.order));
					}
					else {
						self.LoadLocationObjectCollection();
					}
		...
		this.initialize = function () {
            ...
            self.subscriptions.push(self.Events.LocationCollectionLoaded.subscribe(function () {
				...
				if(self.sortColumnName() !== null && !self.multiSortOrderBy) {
					self.LocationObjectCollection.sort(GO.getSortFunction(self.sortColumnName(), self.sortOrder()));
				}
			}));

			// Subscribe to changes on other grids with the same object in order to rebind the current grid
			self.subscriptions.push(ko.postbox.subscribe("Location.ChangedOnGrid", function(payload) {
				// Checking the caller is not the current Grid
				if (payload.gridName !== 'LocationGrid' || JSON.stringify(payload.contextId) != JSON.stringify(self.contextId)) {
					self.Rebind();
				}
			}));
	...
	if (window.ApplicationSourceHandler)
		window.ApplicationSourceHandler.onSourceLoaded("/ViewModels/Location/LocationGridViewModel.js");
} ());
