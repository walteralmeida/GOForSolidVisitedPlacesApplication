...
(function () {
	...
	Solid.Web.ViewModels.UserProfileGridViewModel = function(controller, $gridContainer, sDataBindRoot, $popupContainer, parentContextId, options) {
		...
		// Object Data
		this.UserProfileObjectCollection = ko.observableArray();

		self.subscriptions.push(self.UserProfileObjectCollection.subscribe( function (newValue) {
		...
		this.selectedObject = ko.pureComputed(function () {
            if (this.selectedObjectId() == -1)
                return null;
            return this.controller.ObjectsDataSet.GetObjectByInternalId("UserProfile", this.selectedObjectId());
        }, this);
        ...
        this.StatusData = {
            ...
            IsEmpty: ko.pureComputed( function () {
                return !self.UserProfileObjectCollection() || self.UserProfileObjectCollection().length === 0;
            }),
		...
		// Events
        this.Events = {
			UserProfileCollectionLoaded: ko.observable(false),
			CollectionLoaded: ko.observable(false),
			Rebound : ko.observable(false)
        };

		this.CallAfterSaveRelatedEntity = function () {
			ko.postbox.publish('UserProfile.ChangedOnGrid', { contextId: self.contextId, action: 'add', gridName: 'UserProfileGrid' });
		}

        this.SetUserProfileObjectCollection = function(dataObjectCollection) {
		    GO.log("UserProfileGrid", "Setting new collection");

            self.selectedObjectId(null);

			var currentCollection = self.UserProfileObjectCollection();
            ...
            if(!GenerativeObjects.Web.GetEnvironment().isMobile) {// mobile infinite lists need to keep the full list of objects
				...
				self.UserProfileObjectCollection.removeAll();
			}
		...
		this.Rebind = function(forceExternal) {
			...
			// If we're only working in memory, we simulate a Rebind by reloading observables
				...
				if (self.getSourceCollection) {
					...
					self.SetUserProfileObjectCollection(allObjects);
				}
            }
		...
		this.LoadUserProfileObjectCollection = function (configuration) {
			if (!self.DataStore)		
				return;
			...
			var configuration = $.merge({
				...
				successHandler:self.OnUserProfileObjectCollectionCounted,
		...
		this.OnUserProfileObjectCollectionCounted = function (count) {
			...
			self.LoadPagedUserProfileObjectCollection();
		};
		this.LoadPagedUserProfileObjectCollection = function (configuration) {
			if (!self.DataStore)		
				return;
			...
			var configuration = $.merge({
				...
				successHandler:self.OnUserProfileObjectCollectionLoaded,
				errorHandler: self.ShowError
	        }, configuration || {} );
        ...
        this.OnUserProfileObjectCollectionLoaded = function (objectsLoaded) {
		    GO.log("UserProfileGrid", "UserProfileCollection loaded");
            self.SetUserProfileObjectCollection(objectsLoaded);
			self.StatusData.IsBusy(false);
            // the next line is to force notification of change: this way we emulate event handling
            self.Events.UserProfileCollectionLoaded(!self.Events.UserProfileCollectionLoaded());
		...
		this.gridSettings = {
			...
			dataCollectionName: 'UserProfileObjectCollection',
			dataCollectionObservable: self.UserProfileObjectCollection,
        	...
        	self.subscriptions.push(self.gridSettings.sortingOptions.subscribe(
        	    function( oSort ) {
                  ...
                  if (self.totalPageNumber() < 2 && !self.multiSortOrderBy) {
						self.UserProfileObjectCollection.sort(GO.getSortFunction(oSort.columnName, oSort.order));
					}
					else {
						self.LoadUserProfileObjectCollection();
					}
		...
		this.initialize = function () {
            ...
            self.subscriptions.push(self.Events.UserProfileCollectionLoaded.subscribe(function () {
				...
				if(self.sortColumnName() !== null && !self.multiSortOrderBy) {
					self.UserProfileObjectCollection.sort(GO.getSortFunction(self.sortColumnName(), self.sortOrder()));
				}
			}));

			// Subscribe to changes on other grids with the same object in order to rebind the current grid
			self.subscriptions.push(ko.postbox.subscribe("UserProfile.ChangedOnGrid", function(payload) {
	...
	if (window.ApplicationSourceHandler)
		window.ApplicationSourceHandler.onSourceLoaded("/ViewModels/UserProfile/UserProfileGridViewModel.js");
} ());
