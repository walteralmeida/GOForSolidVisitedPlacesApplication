...
(function () {
	//
	Solid.Web.ViewModels.VisitedPlaceGrid1ViewModel = function(controller, $gridContainer, sDataBindRoot, $popupContainer, parentContextId, options) {
		var self = this;
		...
		if (Solid.Web.ViewModels.VisitedPlaceGrid1ViewModelCustom !== undefined) {
		    this.customViewModel = new Solid.Web.ViewModels.VisitedPlaceGrid1ViewModelCustom(self);
		};
		...
		this.StatusData.Title = ko.pureComputed(function() {
			...
			return self.alternateTitle || "Visitors of the country";
		});
		...
		this.CallAfterSaveRelatedEntity = function () {
			ko.postbox.publish('VisitedPlace.ChangedOnGrid', { contextId: self.contextId, action: 'add', gridName: 'VisitedPlaceGrid1' });
		}

        this.SetVisitedPlaceObjectCollection = function(dataObjectCollection) {
		    GO.log("VisitedPlaceGrid1", "Setting new collection");
		...
		this.LoadPagedVisitedPlaceObjectCollection = function (configuration) {
		    ...
		    GO.log("VisitedPlaceGrid1", "Calling load");
        ...
        this.OnVisitedPlaceObjectCollectionLoaded = function (objectsLoaded) {
		    GO.log("VisitedPlaceGrid1", "VisitedPlaceCollection loaded");
		...
		this.initialize = function () {
			...
			self.subscriptions.push(ko.postbox.subscribe("VisitedPlace.ChangedOnGrid", function(payload) {
				...
				if (payload.gridName !== 'VisitedPlaceGrid1' || JSON.stringify(payload.contextId) != JSON.stringify(self.contextId)) {
					self.Rebind();
				}
			}));
	...
	if (window.ApplicationSourceHandler)
		window.ApplicationSourceHandler.onSourceLoaded("/ViewModels/VisitedPlace/VisitedPlaceGrid1ViewModel.js");
} ());
