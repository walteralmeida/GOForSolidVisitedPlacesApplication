...
{
	public partial class PlaceToLocationService : QueryParamCollection, IDataProvider<PlaceToLocationDataObject>
    {	
		...
		static string _serviceUrl = String.Format("{0}/dataset/api/{1}/", BaseUrl.TrimEnd('/'), "placetolocation");		
		...
		public virtual PlaceToLocationDataObject Save(
			PlaceToLocationDataObject theDataObjectToSave, 
		...
		{
			return Save(new PlaceToLocationContainer(theDataObjectToSave, includeDirtyObjectsOnly), securityFilterExpression, includes, parameters: parameters);
		}
		...
		public virtual PlaceToLocationDataObject Save(
			PlaceToLocationContainer theDataObjectToSave,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null,
				...
				try
				{
					using (var response = request.GetResponse())
					{
						...
						using (var responseStream = response.GetResponseStream())
						{
							...
							var container = JsonConvert.DeserializeObject<PlaceToLocationContainer>(sr.ReadToEnd(), JsonSerializerSettings);

							if (container == null)
							{
								if (theDataObjectToSave.ExtractPlaceToLocation().IsMarkedForDeletion)
									return null;
								else
									throw new GOServerException("No PlaceToLocation in the response to SDK DataProvider Save(). Not expecting a null response here!");
							}
							else
							{
								...
								response.Close();
								return container.ExtractPlaceToLocation();
							}
						}
					}
				}
		...
		public virtual PlaceToLocationDataObject Get(
			PlaceToLocationDataObject theDataObjectToGet,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */) 
		{
			...
			var pksUrl = "";
			pksUrl += Uri.EscapeDataString(theDataObjectToGet.LocationURI.ToString()) + "/";
			pksUrl += Uri.EscapeDataString(theDataObjectToGet.PlaceURI.ToString()) + "/";
			pksUrl.TrimEnd('/');
		...
		public virtual DataObjectCollection<PlaceToLocationDataObject> GetCollection(
			LambdaExpression securityFilterExpression = null, 
			...
			try
			{
				using (var response = request.GetResponse())
				{
					...
					using (var responseStream = response.GetResponseStream())
					{
						...
						using (var sr = new StreamReader(responseStream))
						{
							var container = JsonConvert.DeserializeObject<PlaceToLocationCollectionContainer>(sr.ReadToEnd(), JsonSerializerSettings);
							container.ObjectsDataSet.EnsureInitialized();
							container.ObjectsDataSet.ReconstructIndexes();

							response.Close();
							return container.ExtractPlaceToLocationItems();
						}
					}
				}
			}
		...
		public virtual void Delete(
			PlaceToLocationDataObject theDataObjectToDelete, 
			LambdaExpression securityFilterExpression = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */)
		{
            ...
            // var uri = new Uri(_serviceUrl + "DeletePlaceToLocation.json");
			var uri = new Uri(_serviceUrl);
			...
			using (var stream = request.GetRequestStream())
			{
				string objectToDeleteAsString = JsonConvert.SerializeObject(new PlaceToLocationContainer(theDataObjectToDelete), JsonSerializerSettings);
