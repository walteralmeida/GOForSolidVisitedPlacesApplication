...
{
	public partial class GORoleService : QueryParamCollection, IDataProvider<GORoleDataObject>
    {	
		...
		static string _serviceUrl = String.Format("{0}/dataset/api/{1}/", BaseUrl.TrimEnd('/'), "gorole");		
		...
		public virtual GORoleDataObject Save(
			GORoleDataObject theDataObjectToSave, 
		...
		{
			return Save(new GORoleContainer(theDataObjectToSave, includeDirtyObjectsOnly), securityFilterExpression, includes, parameters: parameters);
		}
		...
		public virtual GORoleDataObject Save(
			GORoleContainer theDataObjectToSave,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null,
				...
				try
				{
					using (var response = request.GetResponse())
					{
						...
						using (var responseStream = response.GetResponseStream())
						{
							...
							var container = JsonConvert.DeserializeObject<GORoleContainer>(sr.ReadToEnd(), JsonSerializerSettings);

							if (container == null)
							{
								if (theDataObjectToSave.ExtractGORole().IsMarkedForDeletion)
									return null;
								else
									throw new GOServerException("No GORole in the response to SDK DataProvider Save(). Not expecting a null response here!");
							}
							else
							{
								...
								response.Close();
								return container.ExtractGORole();
							}
						}
					}
				}
		...
		public virtual GORoleDataObject Get(
			GORoleDataObject theDataObjectToGet,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
		...
		public virtual DataObjectCollection<GORoleDataObject> GetCollection(
			LambdaExpression securityFilterExpression = null, 
			...
			try
			{
				using (var response = request.GetResponse())
				{
					...
					using (var responseStream = response.GetResponseStream())
					{
						...
						using (var sr = new StreamReader(responseStream))
						{
							var container = JsonConvert.DeserializeObject<GORoleCollectionContainer>(sr.ReadToEnd(), JsonSerializerSettings);
							container.ObjectsDataSet.EnsureInitialized();
							container.ObjectsDataSet.ReconstructIndexes();

							response.Close();
							return container.ExtractGORoleItems();
						}
					}
				}
			}
		...
		public virtual void Delete(
			GORoleDataObject theDataObjectToDelete, 
			LambdaExpression securityFilterExpression = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */)
		{
            ...
            // var uri = new Uri(_serviceUrl + "DeleteGORole.json");
			var uri = new Uri(_serviceUrl);
			...
			using (var stream = request.GetRequestStream())
			{
				string objectToDeleteAsString = JsonConvert.SerializeObject(new GORoleContainer(theDataObjectToDelete), JsonSerializerSettings);
