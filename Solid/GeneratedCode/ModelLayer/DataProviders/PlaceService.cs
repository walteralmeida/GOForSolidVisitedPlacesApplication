...
{
	public partial class PlaceService : QueryParamCollection, IDataProvider<PlaceDataObject>
    {	
		...
		static string _serviceUrl = String.Format("{0}/dataset/api/{1}/", BaseUrl.TrimEnd('/'), "place");		
		...
		public virtual PlaceDataObject Save(
			PlaceDataObject theDataObjectToSave, 
		...
		{
			return Save(new PlaceContainer(theDataObjectToSave, includeDirtyObjectsOnly), securityFilterExpression, includes, parameters: parameters);
		}
		...
		public virtual PlaceDataObject Get(
			PlaceDataObject theDataObjectToGet,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
		...
		public virtual DataObjectCollection<PlaceDataObject> GetCollection(
			LambdaExpression securityFilterExpression = null, 
			...
			try
			{
				using (var response = request.GetResponse())
				{
					...
					using (var responseStream = response.GetResponseStream())
					{
						...
						using (var sr = new StreamReader(responseStream))
						{
							var container = JsonConvert.DeserializeObject<PlaceCollectionContainer>(sr.ReadToEnd(), JsonSerializerSettings);
							container.ObjectsDataSet.EnsureInitialized();
							container.ObjectsDataSet.ReconstructIndexes();

							response.Close();
							return container.ExtractPlaceItems();
						}
					}
				}
			}
		...
		public virtual void Delete(
			PlaceDataObject theDataObjectToDelete, 
			LambdaExpression securityFilterExpression = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */)
		{
            ...
            // var uri = new Uri(_serviceUrl + "DeletePlace.json");
			var uri = new Uri(_serviceUrl);
			...
			using (var stream = request.GetRequestStream())
			{
				string objectToDeleteAsString = JsonConvert.SerializeObject(new PlaceContainer(theDataObjectToDelete), JsonSerializerSettings);
