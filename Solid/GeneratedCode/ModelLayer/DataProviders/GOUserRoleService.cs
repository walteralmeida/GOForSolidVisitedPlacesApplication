...
{
	public partial class GOUserRoleService : QueryParamCollection, IDataProvider<GOUserRoleDataObject>
    {	
		...
		static string _serviceUrl = String.Format("{0}/dataset/api/{1}/", BaseUrl.TrimEnd('/'), "gouserrole");		
		...
		public virtual GOUserRoleDataObject Save(
			GOUserRoleDataObject theDataObjectToSave, 
		...
		{
			return Save(new GOUserRoleContainer(theDataObjectToSave, includeDirtyObjectsOnly), securityFilterExpression, includes, parameters: parameters);
		}
		...
		public virtual GOUserRoleDataObject Save(
			GOUserRoleContainer theDataObjectToSave,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null,
				...
				try
				{
					using (var response = request.GetResponse())
					{
						...
						using (var responseStream = response.GetResponseStream())
						{
							...
							var container = JsonConvert.DeserializeObject<GOUserRoleContainer>(sr.ReadToEnd(), JsonSerializerSettings);

							if (container == null)
							{
								if (theDataObjectToSave.ExtractGOUserRole().IsMarkedForDeletion)
									return null;
								else
									throw new GOServerException("No GOUserRole in the response to SDK DataProvider Save(). Not expecting a null response here!");
							}
							else
							{
								...
								response.Close();
								return container.ExtractGOUserRole();
							}
						}
					}
				}
		...
		public virtual GOUserRoleDataObject Get(
			GOUserRoleDataObject theDataObjectToGet,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
		...
		public virtual DataObjectCollection<GOUserRoleDataObject> GetCollection(
			LambdaExpression securityFilterExpression = null, 
			...
			try
			{
				using (var response = request.GetResponse())
				{
					...
					using (var responseStream = response.GetResponseStream())
					{
						...
						using (var sr = new StreamReader(responseStream))
						{
							var container = JsonConvert.DeserializeObject<GOUserRoleCollectionContainer>(sr.ReadToEnd(), JsonSerializerSettings);
							container.ObjectsDataSet.EnsureInitialized();
							container.ObjectsDataSet.ReconstructIndexes();

							response.Close();
							return container.ExtractGOUserRoleItems();
						}
					}
				}
			}
		...
		public virtual void Delete(
			GOUserRoleDataObject theDataObjectToDelete, 
			LambdaExpression securityFilterExpression = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */)
		{
            ...
            // var uri = new Uri(_serviceUrl + "DeleteGOUserRole.json");
			var uri = new Uri(_serviceUrl);
			...
			using (var stream = request.GetRequestStream())
			{
				string objectToDeleteAsString = JsonConvert.SerializeObject(new GOUserRoleContainer(theDataObjectToDelete), JsonSerializerSettings);
