...
{
	public partial class GOLoginHistoryService : QueryParamCollection, IDataProvider<GOLoginHistoryDataObject>
    {	
		...
		static string _serviceUrl = String.Format("{0}/dataset/api/{1}/", BaseUrl.TrimEnd('/'), "gologinhistory");		
		...
		public virtual GOLoginHistoryDataObject Save(
			GOLoginHistoryDataObject theDataObjectToSave, 
		...
		{
			return Save(new GOLoginHistoryContainer(theDataObjectToSave, includeDirtyObjectsOnly), securityFilterExpression, includes, parameters: parameters);
		}
		...
		public virtual GOLoginHistoryDataObject Save(
			GOLoginHistoryContainer theDataObjectToSave,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null,
				...
				try
				{
					using (var response = request.GetResponse())
					{
						...
						using (var responseStream = response.GetResponseStream())
						{
							...
							var container = JsonConvert.DeserializeObject<GOLoginHistoryContainer>(sr.ReadToEnd(), JsonSerializerSettings);

							if (container == null)
							{
								if (theDataObjectToSave.ExtractGOLoginHistory().IsMarkedForDeletion)
									return null;
								else
									throw new GOServerException("No GOLoginHistory in the response to SDK DataProvider Save(). Not expecting a null response here!");
							}
							else
							{
								...
								response.Close();
								return container.ExtractGOLoginHistory();
							}
						}
					}
				}
		...
		public virtual GOLoginHistoryDataObject Get(
			GOLoginHistoryDataObject theDataObjectToGet,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
		...
		public virtual DataObjectCollection<GOLoginHistoryDataObject> GetCollection(
			LambdaExpression securityFilterExpression = null, 
			...
			try
			{
				using (var response = request.GetResponse())
				{
					...
					using (var responseStream = response.GetResponseStream())
					{
						...
						using (var sr = new StreamReader(responseStream))
						{
							var container = JsonConvert.DeserializeObject<GOLoginHistoryCollectionContainer>(sr.ReadToEnd(), JsonSerializerSettings);
							container.ObjectsDataSet.EnsureInitialized();
							container.ObjectsDataSet.ReconstructIndexes();

							response.Close();
							return container.ExtractGOLoginHistoryItems();
						}
					}
				}
			}
		...
		public virtual void Delete(
			GOLoginHistoryDataObject theDataObjectToDelete, 
			LambdaExpression securityFilterExpression = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */)
		{
            ...
            // var uri = new Uri(_serviceUrl + "DeleteGOLoginHistory.json");
			var uri = new Uri(_serviceUrl);
			...
			using (var stream = request.GetRequestStream())
			{
				string objectToDeleteAsString = JsonConvert.SerializeObject(new GOLoginHistoryContainer(theDataObjectToDelete), JsonSerializerSettings);
