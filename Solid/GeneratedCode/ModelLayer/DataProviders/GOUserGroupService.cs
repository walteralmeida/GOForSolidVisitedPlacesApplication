...
{
	public partial class GOUserGroupService : QueryParamCollection, IDataProvider<GOUserGroupDataObject>
    {	
		...
		static string _serviceUrl = String.Format("{0}/dataset/api/{1}/", BaseUrl.TrimEnd('/'), "gousergroup");		
		...
		public virtual GOUserGroupDataObject Save(
			GOUserGroupDataObject theDataObjectToSave, 
		...
		{
			return Save(new GOUserGroupContainer(theDataObjectToSave, includeDirtyObjectsOnly), securityFilterExpression, includes, parameters: parameters);
		}
		...
		public virtual GOUserGroupDataObject Save(
			GOUserGroupContainer theDataObjectToSave,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null,
				...
				try
				{
					using (var response = request.GetResponse())
					{
						...
						using (var responseStream = response.GetResponseStream())
						{
							...
							var container = JsonConvert.DeserializeObject<GOUserGroupContainer>(sr.ReadToEnd(), JsonSerializerSettings);

							if (container == null)
							{
								if (theDataObjectToSave.ExtractGOUserGroup().IsMarkedForDeletion)
									return null;
								else
									throw new GOServerException("No GOUserGroup in the response to SDK DataProvider Save(). Not expecting a null response here!");
							}
							else
							{
								...
								response.Close();
								return container.ExtractGOUserGroup();
							}
						}
					}
				}
		...
		public virtual GOUserGroupDataObject Get(
			GOUserGroupDataObject theDataObjectToGet,
			LambdaExpression securityFilterExpression = null, 
			List<string> includes = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */) 
		{
			...
			pksUrl += Uri.EscapeDataString(theDataObjectToGet.GOUserId.ToString()) + "/";
			pksUrl.TrimEnd('/');
		...
		public virtual DataObjectCollection<GOUserGroupDataObject> GetCollection(
			LambdaExpression securityFilterExpression = null, 
			...
			try
			{
				using (var response = request.GetResponse())
				{
					...
					using (var responseStream = response.GetResponseStream())
					{
						...
						using (var sr = new StreamReader(responseStream))
						{
							var container = JsonConvert.DeserializeObject<GOUserGroupCollectionContainer>(sr.ReadToEnd(), JsonSerializerSettings);
							container.ObjectsDataSet.EnsureInitialized();
							container.ObjectsDataSet.ReconstructIndexes();

							response.Close();
							return container.ExtractGOUserGroupItems();
						}
					}
				}
			}
		...
		public virtual void Delete(
			GOUserGroupDataObject theDataObjectToDelete, 
			LambdaExpression securityFilterExpression = null, 
			IObjectsDataSet context = null, 
			Parameters parameters = null, 
			bool skipSecurity = false /* skipSecurity has no effect here */)
		{
            ...
            // var uri = new Uri(_serviceUrl + "DeleteGOUserGroup.json");
			var uri = new Uri(_serviceUrl);
			...
			using (var stream = request.GetRequestStream())
			{
				string objectToDeleteAsString = JsonConvert.SerializeObject(new GOUserGroupContainer(theDataObjectToDelete), JsonSerializerSettings);
